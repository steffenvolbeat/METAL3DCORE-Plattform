...
function Galaxy({
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const angle = Math.random() * Math.PI * 8; // Mehrere Spiralarme
    ...
    // Farb-Gradient von innen nach außen
  ...
  // Galaxie-Partikel Geometry erstellen
  ...
  useFrame((state) => {
    ...
    if (particlesRef.current) {
      ...
      particlesRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 2) * (beatData.beat ? beatData.intensity * 0.5 : 0);
      particlesRef.current.rotation.z = Math.cos(state.clock.elapsedTime * 3) * (beatData.beat ? beatData.intensity * 0.3 : 0);
    }
  });
    ...
    if (orbitRef.current) {
      ...
      children.forEach((child) => {
        if (
          ...
          if (beatData.beat) {
            child.material.emissive = new THREE.Color("#FF6B35").multiplyScalar(
              beatData.intensity
            );
          } else {
  ...
  useFrame((state) => {
    if (systemRef.current) {
      ...
      systemRef.current.position.x =
        Math.sin(state.clock.elapsedTime * 0.1) * 5;
      systemRef.current.position.z =
        Math.cos(state.clock.elapsedTime * 0.1) * 3;
      ...
      if (beatData.beat && beatData.intensity > 0.7) {
        ...
        systemRef.current.position.z = THREE.MathUtils.lerp(
          systemRef.current.position.z,
          targetZ,
          0.3
        );
