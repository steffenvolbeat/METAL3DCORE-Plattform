    ...
    <group ref={lightRef}>
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <group key={i} rotation={[0, (i * Math.PI * 2) / 8, 0]}>
          <pointLight position={[5, 6, 0]} color={`hsl(${i * 45}, 100%, 70%)`} intensity={0.3} distance={8} />
        </group>
      ))}
    </group>
  );
...
function MusicVisualizer({ isPlaying }: { isPlaying: boolean }) {
  ...
  const intensitiesRef = useRef<number[]>(Array.from({ length: EQUALIZER_BAR_COUNT }, () => 0.4));

  useEffect(() => {
    // Ensure ref arrays stay aligned with the rendered bar count
    if (barRefs.current.length !== EQUALIZER_BAR_COUNT) {
      barRefs.current = Array.from({ length: EQUALIZER_BAR_COUNT }, (_, index) => barRefs.current[index] ?? null);
    }

    if (intensitiesRef.current.length !== EQUALIZER_BAR_COUNT) {
      intensitiesRef.current = Array.from({ length: EQUALIZER_BAR_COUNT }, () => 0.4);
    }
  }, []);
  ...
  useFrame(state => {
    ...
    if (intensitiesRef.current.length !== EQUALIZER_BAR_COUNT) {
      intensitiesRef.current = Array.from({ length: EQUALIZER_BAR_COUNT }, () => idleHeight);
    }
    ...
    for (let i = 0; i < EQUALIZER_BAR_COUNT; i += 1) {
      ...
      const targetScale = isPlaying ? idleHeight + (waveA * 1.6 + waveB * 1.1 + waveC * 0.9) : idleHeight;
  ...
  return (
    <group ref={groupRef} position={[-14.5, 3, 0]} rotation={[0, Math.PI / 2, 0]}>
      ...
      <Text position={[0, 3.5, 0.25]} fontSize={0.4} color="#00FF00" anchorX="center" anchorY="middle">
        🎵 AUDIO EQUALIZER 🎵
      </Text>
    </group>
  );
  ...
  return (
    <div className="w-[360px] rounded-2xl border border-blue-400/60 bg-gradient-to-br from-blue-950/90 via-slate-950/85 to-black/90 shadow-[0_20px_45px_rgba(0,0,0,0.45)] backdrop-blur-xl transition-all duration-300">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-blue-400/40">
        <h3 className="text-blue-200 font-semibold text-xl tracking-wide">📺 TV Control Deck</h3>
        <button
      ...
      {!isCollapsed && (
        <div className="px-5 py-5 space-y-4">
          ...
          <div className="pt-2">
            <label className="block text-blue-200 text-sm font-semibold mb-2">📹 Eigene YouTube URL</label>
  ...
  return (
    <div className="w-[360px] rounded-2xl border border-purple-400/60 bg-gradient-to-br from-purple-950/92 via-slate-950/85 to-black/90 shadow-[0_24px_48px_rgba(0,0,0,0.45)] backdrop-blur-xl transition-all duration-300">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-purple-400/40">
        <h3 className="text-purple-200 font-semibold text-xl tracking-wide">🎭 Gallery Director</h3>
        <button
          onClick={onToggleCollapse}
      ...
      {!isCollapsed && (
        <div className="max-h-[calc(100vh-160px)] w-[350px] overflow-y-auto px-6 py-6 space-y-6">
          <div className="flex items-center justify-center">
            <button
              onClick={() => setLoungeMode(loungeMode === "relaxed" ? "interview" : "relaxed")}
          ...
          <div className="rounded-xl border border-slate-500/50 bg-gradient-to-br from-slate-900/80 via-black/70 to-slate-900/80 p-5 shadow-inner">
            <h4 className="mb-3 text-lg font-semibold text-slate-200">🖼️ FRONT GALLERY</h4>
            <div className="mb-2 text-center text-sm text-slate-300">19 Frames - Front Wall</div>
          ...
          <div className="rounded-xl border border-blue-400/50 bg-gradient-to-br from-blue-900/80 via-slate-900/80 to-black/70 p-5 shadow-inner">
            <h4 className="mb-3 text-lg font-semibold text-blue-200">📺 GIANT TV</h4>
            <div className="mb-2 text-center text-sm text-blue-200/90">Back Wall TV - 19.5m x 7.5m</div>
            <div className="text-center text-xs text-blue-300/90">📍 Full Back Wall Coverage</div>
          </div>
          ...
          <div className="rounded-xl border border-purple-400/50 bg-gradient-to-br from-purple-900/80 via-slate-900/80 to-black/70 p-5 shadow-inner">
            <h4 className="mb-3 text-lg font-semibold text-purple-200">🎵 AMBIANCE</h4>
            ...
            <div className="text-center font-semibold text-purple-100">{ambiance}% Comfortable</div>
          </div>

          <div className="rounded-xl border border-purple-400/40 bg-slate-950/80 p-5 shadow-inner">
            <h4 className="mb-4 text-lg font-semibold text-purple-200">🏆 VIP SERVICES</h4>
...
function BackstageControlStation({
  ...
  return (
    <group position={[14.7, 2.6, 0]} rotation={[0, -Math.PI / 2, 0]}>
      <Html transform sprite={false} distanceFactor={8} occlude={false} style={{ pointerEvents: "auto" }}>
        <div className="flex w-[380px] flex-col space-y-4">
          <div className="rounded-2xl border border-purple-500/60 bg-gradient-to-br from-purple-950/95 via-black/90 to-slate-950/90 px-5 py-4 shadow-[0_22px_45px_rgba(0,0,0,0.45)] backdrop-blur-xl">
            ...
            <div className="mt-3 text-[0.65rem] text-purple-100/70">
              ...
              <span className="font-semibold text-purple-200"> G</span> Gallery | Panels collapsible
              {isFullscreen && " | Exit = ⬅️"}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            {showBackstageControl && (
              <BackstageControlPanel
                ...
                onToggleCollapse={() => setGalleryControlCollapsed(prev => !prev)}
              />
            )}
          </div>
        </div>
...
function BackstageEnvironment({
  ...
  return (
    <group>
      {/* FLOOR */}
      <Plane args={[30, 22]} rotation={[-Math.PI / 2, 0, 0]} position={[0, -2, 0]} receiveShadow>
      ...
      <Plane args={[22, 10]} position={[-15, 3, 0]} rotation={[0, Math.PI / 2, 0]} receiveShadow>
      ...
      {/* CEILING */}
      <Plane args={[30, 22]} position={[0, 8, 0]} rotation={[Math.PI / 2, 0, 0]} receiveShadow>
      ...
      <group position={[0, -1, 2]}>
        <Box args={[6, 1.5, 2]} position={[0, 0.75, 0]} castShadow>
          <meshStandardMaterial color="#8B008B" roughness={0.1} metalness={0.2} />
        </Box>
      ...
      <group position={[0, -1.5, -1]}>
        <Box args={[3, 0.3, 1.5]} position={[0, 0.4, 0]} castShadow>
          <meshStandardMaterial color="#654321" roughness={0.2} metalness={0.1} />
        </Box>
      ...
      {/* FRONT WALL GALLERY */}
      <group position={[0, 3, 10.8]}>
        <TextureReadyFrame position={[0, 0, 0]} size={[3, 2.5]} imageId="main-featured" placeholderColor="#FF6347" />
        ...
        {Array.from({ length: 5 }).map((_, i) => (
          <TextureReadyFrame
            ...
            size={[1.2, 1.5]}
            imageId={`top-row-${i + 1}`}
            placeholderColor={i % 3 === 0 ? "#FF4500" : i % 3 === 1 ? "#9370DB" : "#20B2AA"}
          />
        ))}
        ...
        {Array.from({ length: 6 }).map((_, i) => (
          <TextureReadyFrame
            ...
            placeholderColor={i % 3 === 0 ? "#DC143C" : i % 3 === 1 ? "#00CED1" : "#FFD700"}
          />
        ))}
      </group>
      ...
      {/* POINT LIGHTS */}
      <pointLight position={[-5, 5, 0]} intensity={1.5} color="#FFE4B5" distance={15} />
      <pointLight position={[5, 5, 0]} intensity={1.5} color="#FFE4B5" distance={15} />
      <pointLight position={[0, 5, -5]} intensity={2.0} color="#F0E68C" distance={15} />
      <pointLight position={[0, 5, 5]} intensity={2.0} color="#F0E68C" distance={15} />

      {/* EFFECTS */}
      <Sparkles count={100} scale={[15, 8, 10]} size={3} speed={0.5} opacity={0.6} color="#FFD700" />

      <Stars radius={20} depth={50} count={200} factor={4} saturation={0.8} fade={true} speed={0.5} />
...
// MAIN COMPONENT
export default function BackstageRoom({ isFullscreen = true, onRoomChange }: BackstageRoomProps) {
  ...
  return (
    ...
    >
      ...
      >
        <WebGLCanvasWrapper
          roomName="Backstage VIP Experience"
          roomIcon="🎭"
        ...
        >
          ...
          >
            ...
            >
              <BackstageEnvironment showYouTubePlayer={showYouTubePlayer} currentVideoId={currentVideoId} />
        ...
        {!isFullscreen && (
          <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-20">
            <div className="bg-black/95 backdrop-blur-md rounded-xl px-8 py-6 border-2 border-purple-500">
              <p className="text-purple-300 text-lg text-center mb-4 font-bold">🎭 BACKSTAGE LOUNGE 🎭</p>
              {fpsMode && (
                <div className="text-center mb-4 text-sm text-blue-300 space-y-1">
                  <p>
                    🖱️ <strong>Click to Lock Mouse</strong> | 🎮 <strong>WASD</strong> = Move |{" "}
        ...
        <div className="absolute inset-0 pointer-events-none z-10 opacity-10">
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
            <div className="text-purple-400/20 text-6xl font-bold">🎭 VIP BACKSTAGE EXPERIENCE 🎭</div>
          </div>
        </div>
      </div>
    </RoomAccessControl>
